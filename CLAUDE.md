...
- **Board model**: `board` is a `ROWS × COLS` (20×10) matrix of cells; each cell is `0` (empty) or a piece-color index `1–7`.
- **Pieces**: `PIECES` defines the 7 tetrominoes as square matrices (color index per cell). `randomPiece()` deep-copies a shape and spawns it centered at the top. Rotation (`rotateCW`) is a transpose + row-reverse; `tryRotate()` applies it with wall-kick offsets `[0, -1, 1, -2, 2]`, taking the first offset that doesn't collide.
