...
- **Visual skins**: `SKINS` is a registry of palettes (`retro`/`neon`/`pastel`/`pixel`), each an array of the same length/index scheme as the old flat `COLORS` array (`0` empty, `1`–`8` piece colors, `9` bomb color). `activeSkin` (module-level `let`, default `'retro'`) selects the active entry; `currentColors()` returns `SKINS[activeSkin].colors`. `BOMB_COLOR` is derived from `SKINS.retro.colors.length - 1` (all palettes share that length). `drawBlock` looks up colors via `currentColors()` and branches on `activeSkin` to one of `drawRetroBlock`/`drawNeonBlock`/`drawPastelBlock`/`drawPixelBlock` (flat fill+highlight, glow via `shadowBlur`/`shadowColor` reset to `0` after each block, rounded rects via `context.roundRect`, and a 3×3 alternating-shade texture, respectively); `drawBombBlock`/`drawExplosion`/`drawGrid` are shared and unaffected by skin. `applySkin(skin)` sets `activeSkin`, syncs the `#skin-select` value, and re-runs `draw()`/`drawNext()` immediately; the choice persists to `localStorage` (`tetris-skin`) and is restored before the first `init()`, mirroring the `applyTheme` pattern.
...
`COLS`, `ROWS`, `BLOCK`, `SKINS`, `LINE_SCORES`, `dropInterval`, `BOMB_LINES_INTERVAL`, `BOMB_SCORE_PER_CELL`, `EXPLOSION_DURATION`. If `COLS`/`ROWS`/`BLOCK` change, update the `#board` canvas `width`/`height` in `index.html` to match (`COLS×BLOCK` by `ROWS×BLOCK`).
